...
// ──────────────────────────────────────────
// 発見バーストの派手さ（連鎖演出 A5 / ラスト演出 B6）
// ──────────────────────────────────────────

/// 「最後の 1 個」を発見したときのバースト派手さ。連鎖の上限より必ず大きく、
/// シーン中で最も豪華な一発になるようにする（B6: やり遂げた瞬間の山場）。
const double kGrandFinaleBurstIntensity = 2.0;

/// 連続発見（連鎖）数 [streak] に応じた発見バーストの派手さ倍率。
///
/// 1 個目は等倍（1.0）で、続けて見つけるほど少しずつ盛り上がる（上限 1.6）。
/// **連鎖が途切れても罰しない**: 途切れ（空振り）は呼び出し側で [streak] を 0 に
/// 戻すだけで、減点・崩壊音・×は一切出さない（no-fail を厳守）。
double comboBurstScale(int streak) {
  if (streak <= 1) {
    return 1.0;
  }
  return (1.0 + 0.15 * (streak - 1)).clamp(1.0, 1.6);
}
