...
> 複数ワールド（12 シーン）・アイドルヒント・ハードモード・フリーモードは**実装済み**（§10）。
...
lib/
  ...
  scenes_catalog.dart             # ホームに並べるシーン一覧（12 ワールド・解放/ハード進行ロジック）
...
assets/
  scenes/scene01.json 〜 scene12.json  # シーン定義（隠し宝の正規化 Rect・ダミー）
...
- [x] **複数ワールド**（12 シーン）+ 解放チェーン + 羊皮紙の宝の地図 + 生きた背景
