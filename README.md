...
> 📖 **公開について**: 個人開発プロジェクトを**コード閲覧・参考用**に公開しています。
> コードは**全権利留保**（[LICENSE](LICENSE) / 無断の利用・複製・再配布は不可）。
> アプリは**データ収集ゼロ設計**（第三者送信なし・行動広告/解析 SDK なし）。

> **コアは「シーク＆ファインド（タップで探す）」。迷路ではない。**
> 1 枚のシーンから隠し宝をタップ（または指/ペンでなぞって）探す → 図鑑に収納 →
...
未就学〜低学年（5〜11 歳）向けの、読み書き不要で遊べる宝探し。ホーム画面を「宝の地図」
（羊皮紙風の世界地図）に見立て、**9 つのワールド**（もり/うみ/そら/やま/よる/さばく/うちゅう/
うみのなか/ゆきやま）を曲線ルートでつないだステージとして配置する。

**起動時にセーブスロット選択**（固定 3 枠）。空きスロットは白紙で、開始時に**絵文字アバターを
選んで**識別する。兄弟など複数人がそれぞれ独立した進捗で遊べる。選んだスロットの「宝の地図」に
入る。すべて解放済みで遊べる**フリーモード**専用枠もある。

**コアループ**: 探す → タップ（または指/ペンでなぞって）見つける → ジューシーに祝う
（拡大 + キラッ + 音）→ 図鑑に収納 → コンプリート → 次のエリア解放。

**むずかしいモード**: 全ワールドをクリアすると解放。宝アイコンが小さくなり、探す量が増え、
未発見の宝が周期的に消えたり現れたりする（点滅）。失敗を罰しない原則は維持（消失中の宝の
タップは無反応・残り 1 つは点滅停止）。
...
- **ミニゲーム拡張**: 迷路 / 間違い探し / タイピング 等を「ミニゲームコレクション」として
  追加する構想（当初コンセプト）。
- 保護者ゲート本体（算数問題ダイアログ）、スター/カスタマイズ/実績（年齢帯で段階導入）。

> 複数ワールド（13 シーン）・アイドルヒント・ハードモード・フリーモードは**実装済み**（§10）。
...
| ルーティング | **go_router** | `/slots`(初期)・`/`・`/hunt/:sceneId`（`?mode=hard` で難化）・`/settings`。スロット未選択なら `/slots` へ redirect |
...
```
lib/
  main.dart                       # SharedPreferences ロード + ProviderScope
  app.dart                        # MaterialApp.router + KidsTheme + locale
  router.dart                     # routerProvider: /slots(初期) , / , /hunt/:sceneId , /settings
  providers.dart                  # prefs/repos/audio/locale/found/scene/activeSlot/saveSlot の Provider 群
  scenes_catalog.dart             # ホームに並べるシーン一覧（13 ワールド・解放/ハード進行ロジック）
  save_slots_catalog.dart         # セーブスロット定義（固定3）+ 絵文字アバター白リスト + フリーモード枠
  features/
    save_slots/                   # 起動時のスロット選択（白紙→絵文字選択・続き/リセット・フリーモード）
    treasure_map/                 # ホーム = 宝の地図（羊皮紙ルート・解放/クリア表示・ハード切替）
    seek_find/                    # ★コア = 隠しオブジェクト探し（Tap/なぞり）
      seek_find_screen.dart       #   InteractiveViewer + 図鑑 + アイドルヒント + 完了演出
      seek_find_logic.dart        #   findHitTargetId / 点滅 opacity（純Dart・テスト対象）
      hard_mode.dart              #   GameMode・ハード用シーン生成（小アイコン化・ダミー昇格）
      scene_background.dart       #   生きた背景（環境アニメ）
      models/  widgets/
    settings/                     # 言語切替 + 保護者ゲート入口 stub
  data/                           # ProgressRepository(slotスコープ) / SettingsRepository / SaveSlotRepository
  shared/                         # audio / strings / theme / widgets
assets/
  scenes/scene01.json 〜 scene13.json  # シーン定義（隠し宝の正規化 Rect・ダミー）
  sfx/found.wav, complete.wav     # 効果音（現状はプレースホルダ無音）
test/  unit/  widget/             # 約 200 テスト（hit-test / 点滅 / repository / スロット / 画面 / 完了 など）
```
...
- **テスト**（`fvm flutter test`, 約 200）
  - Unit: `findHitTargetId`（座標正規化 + `Rect.contains` + 点滅 `hiddenIds`）/ `treasureBlinkOpacity`（ハード点滅）/ ハードモードシーン生成・整合 / `ProgressRepository`（slotスコープ・ハード名前空間） / `SettingsRepository` / `SaveSlotRepository` / `SaveSlotController`（スロット独立・生成/リセット・絵文字アバター） / locale / strings / アイコン衝突
  - Widget: スロット選択（白紙→絵文字選択→遷移 / リセット→保護者ゲート / フリーモード）/ 宝の地図ホーム / 絵文字ピッカー / `KidsButton` / `CollectionBar` / `FoundBurst` / 起動スモーク（`/slots`）
  - **`seek_find_screen` の widget テストは skip**（`FoundBurst` アニメ + `rootBundle` + ジェスチャが flutter_test 上で相互干渉・ハングするため）。発見/点滅ロジックは unit で担保し、画面は実機で確認
...
- [x] **MVP**: 基盤スケルトン + シーク＆ファインド（Tap/なぞり）コアループ ✅
- [x] fvm でバージョン統一（Flutter 3.44.2） / Android Firebase App Distribution ワークフロー
- [x] **セーブスロット**（固定3・進捗のみ独立・起動時に絵文字アバター選択・リセットは保護者ゲート）
- [x] **複数ワールド**（13 シーン）+ 解放チェーン + 羊皮紙の宝の地図 + 生きた背景
- [x] **アイドルヒント**（無操作で未発見の宝がわずかに光る）+ カウントアップ図鑑 + ダミー/ターゲットのアイコン衝突防止
- [x] **フリーモード**（全マップ解放）/ **ハードモード**（全クリア後解放・小アイコン・探す量増・宝の点滅）
- [ ] 実機（Android タブレット / iPad）での DoD 手動確認（手順は [`docs/development.md`](docs/development.md) §3）
- [ ] 実シーンのイラスト + 効果音アセット差し替え（現状はグラデーション + 無音のプレースホルダ）
- [ ] **Dig（掘る）操作**の追加（同コアループ上に内部差し替え）
- [ ] 保護者ゲート本体（算数問題ダイアログ・現状は入口 stub）
...
- **コード**: 全権利留保（[LICENSE](LICENSE)）。閲覧・参考用の公開で、無断の利用・複製・再配布は不可。
