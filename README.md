...
**起動時にセーブスロット選択**（固定 3・アバターで識別）。兄弟など複数人がそれぞれ独立した
進捗で遊べる。選んだスロットの「宝の地図」に入る。
...
| ルーティング | **go_router** | `/slots`(初期)・`/`・`/hunt/:sceneId`・`/settings` |
...
lib/
  ...
  save_slots_catalog.dart         # セーブスロット定義（固定3・固定アバター）
  features/
    save_slots/                   # 起動時のスロット選択（アバター識別・新規/続き/リセット）
...
test/  unit/  widget/             # 30 テスト（hit-test / repository / スロット / 画面 / 完了 など）
```
...
- **テスト**（`fvm flutter test`, 計 30）
  - Unit: `findHitTargetId`（座標正規化 + `Rect.contains`）/ `ProgressRepository`（slotスコープ） / `SettingsRepository` / `SaveSlotRepository` / `SaveSlotController`（スロット独立・生成/リセット） / locale / strings
  - Widget: スロット選択（新規→遷移 / リセット→保護者ゲート）/ 宝の地図ホーム / シーン画面（タップ発見 → 図鑑充填 → コンプリート / 再入リセット）/ `KidsButton` / `CollectionBar` / `FoundBurst` / 起動スモーク（`/slots`）
...
- [x] **セーブスロット**（固定3・進捗のみ独立・起動時アバター選択・リセットは保護者ゲート）
