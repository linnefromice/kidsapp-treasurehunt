...
> 複数ワールド（9 シーン）・アイドルヒント・ハードモード・フリーモードは**実装済み**（§10）。
...
lib/
  ...
  scenes_catalog.dart             # ホームに並べるシーン一覧（9 ワールド・解放/ハード進行ロジック）
...
assets/
  scenes/scene01.json 〜 scene09.json  # シーン定義（隠し宝の正規化 Rect・ダミー）
...
- [x] **複数ワールド**（9 シーン）+ 解放チェーン + 羊皮紙の宝の地図 + 生きた背景
