...
2. 白紙スロットをタップ → **絵文字ピッカー**（**66 種**・ボタン状のセル）が開く。1 つ選ぶとそのアバターでスロット作成 + ホームへ。作成済みスロットの**編集ボタン**でアバターを後から変更できる（非破壊・保護者ゲートなし）。
...
10. ホームに **やさしい / ふつう / むずかしい トグル**が**常時表示**。選んだ難易度は**ナビゲーションやアプリ再起動をまたいで保持**される（クリアして地図に戻っても easy に戻らない）。Hard では宝もおとりも数が増え・**周期的に点滅**（消える/現れる）する。消失中の宝のタップは無反応。残り 1 つは点滅停止。
11. 設定で English に切替 → UI が英語化し、再起動後も保持（言語は全スロット共通）。
12. 設定の**「なぞった ときの いろ」**でスタイルを選ぶ → **たんしょく**は **20 色**から 1 色、**にじ（3しょく）**はドロップダウン 3 つで選んだ色を循環、**にじ（フル）**は色相が流れる。なぞりキラキラに反映され、再起動後も保持（全スロット共通）。
13. **Normal / Hard** のシーンで **「うごかす / なぞる」トグル**を確認 → 「うごかす」はドラッグで地図パン・タップで発見、「なぞる」はドラッグでなぞり発見。どちらでもパンと発見が誤爆しない。
14. **連続で発見**するとバーストが少しずつ派手に（空振りで静かにリセット・罰しない）。**最後の 1 個**は一番豪華に弾け、**クリア画面で紙吹雪**が一度降る。
15. **クリア済みシーンを再訪**（またはフリーモード）すると宝・おとりの**配置が毎回変わる**。初回プレイ（未クリア）は安定配置のまま。
16. 宝をいくつか見つける → ホームの**📖（ずかん）**から **図鑑画面**へ → 見つけた宝は**カラー**・未発見は**影絵**、初発見に **NEW** バッジ、上部に「**あつめた n/total**」。一度図鑑を開くと NEW は消える。再起動後も保持。スロットを変えると別の図鑑。
