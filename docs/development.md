...
1. 起動するとスロット選択画面（アバター 3 つ・すべて「あたらしく」）。
2. 1 つ選ぶと進捗が初期化され、ホームに 3 枚のシーンカード（`scene01` 解放・他ロック）。
3. `scene01` → シーンに宝アイコン（りんご/とり/ほし）が見える。図鑑バーにも未発見アイコン（グレー）が並ぶ。
4. 宝を**タップ**、または指/ペンで**なぞる**と発見（点灯 + 図鑑が埋まる）。空振りは無反応。3 個すべて発見 → 「みつけたね！」。ホームに戻ると `scene01` に ✓。
5. アプリ再起動 → スロット選択でそのスロットが「つづき」表示、選ぶと続きから。別スロットを新規で選ぶと**独立した進捗**で始まる。
6. 作成済みスロットのゴミ箱 → 保護者ゲート → OK でリセット（「あたらしく」に戻る）。
7. 設定で English に切替 → UI が英語化し、再起動後も保持（言語は全スロット共通）。
