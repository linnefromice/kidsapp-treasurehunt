...
```
User: /plan マーケットが解決された時にリアルタイム通知を追加する必要があります

Agent (planner):
# 実装計画: リアルタイムマーケット解決通知

## 要件の再述
- ウォッチしているマーケットが解決された時にユーザーに通知を送信
...
> **より深い計画が必要ですか？** PRD統合、コードベース分析、パターン抽出を含むアーティファクト生成型計画には `/prp-plan` を使用してください。厳密な検証ループでそれらの計画を実行するには `/prp-implement` を使用してください。

## 関連エージェント

このコマンドはECCが提供する `planner` エージェントを呼び出します。
